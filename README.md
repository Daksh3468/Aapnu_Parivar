...
Aapnu Parivar is an end-to-end, enterprise-grade digital government portal engineered for Gujarat State to deliver single-window entitlement access across state and central welfare schemes. By shifting benefit eligibility from individual silos to a structured household identity, Aapnu Parivar eliminates duplicate beneficiaries, prevents welfare exclusion, automates document verification, and gracefully handles complex real-world household dynamics (such as nuclear household splits, births, and marriages) with complete lineage retention.

---

## 🌟 Comprehensive Project Overview & Architecture

### 1. The Core Problem Statement & Vision
In traditional state welfare management:
- Every government department maintains isolated databases and demands repeated submission of identical identity/income documents.
- Citizens frequently miss out on entitled benefits due to lack of visibility ("Exclusion Gap").
- Ineligible individuals or duplicate registrations drain state resources.
- Nuclear family divisions (household splits) lead to fragmented, untraceable records.

**Aapnu Parivar** solves this by establishing a **single, verified, 12-digit Gujarat Family ID** (`GJ-DD-YY-SSSSSSS-C`) protected by the Verhoeff checksum algorithm. It provides:
1. **Unified Authentication Portal**: Single sign-in portal for citizens (10-digit mobile & password or Google sign-in) and state officials (`admin@gujarat.gov.in`), with automatic role-based routing (`/my-family` vs `/officer`). First-time family registrants are prompted to establish a secure login password upon initial login.
2. **Automated Scheme Eligibility Engine**: Instant rule evaluation across scheme catalogs using verified profile parameters (income band, ration card type, land holdings, social category, and document status).
3. **Lineage-Aware Household Split Engine**: 4-step wizard allowing citizens to form independent households. Automated Anomaly Risk Scoring (0–100) auto-approves low-risk splits while routing high-risk requests to officer review queues.
4. **Aapnu Mitra AI Chatbot Widget**: Interactive floating chatbot with high-contrast styling (`text-amber-400` on dark slate background) for 24/7 assistance on eligibility rules, split procedures, and document verification.
5. **Departmental Officer Console**: Jurisdiction-scoped access (State, District, Taluka) with live KPIs, Plotly performance analytics, document verification queues, and split approval workflows.
6. **Synthetic Gujarat Dataset**: Built-in synthetic generator producing 150+ realistic household records across Gujarat districts for robust testing and demonstration.

---

### 2. Detailed Technical Architecture & Modules

#### Backend (FastAPI + SQLAlchemy + Pydantic v2)
- **Modular Routers** (`backend/app/api/v1/`):
  - `auth.py`: Common login, JWT authentication, password updates for first-time family logins, role routing.
  - `families.py`: Household creation, member additions, Verhoeff Family ID generation, detail updates.
  - `events.py`: Life event processing (household splits with anomaly risk scoring, births, deaths).
  - `schemes.py`: Scheme catalog querying, YAML rule evaluation, eligibility group categorization (Eligible, Action Needed, Ineligible).
  - `chatbot.py`: AI query endpoint powering the Aapnu Mitra chatbot widget.
  - `admin.py`: Departmental officer metrics, jurisdiction filtering, split approval/rejection endpoints.
- **Rules Engine** (`backend/app/rules/`): Declarative YAML rules evaluator computing eligibility based on real household facts.
- **Verhoeff Generator** (`backend/app/utils/family_id.py`): Ensures strict 12-digit format with checksum validation.

#### Frontend (React + TypeScript + Vite + Tailwind CSS)
- **Unified Login Portal** (`LoginPage.tsx`): Dual tab interface supporting Mobile/Password or Google OAuth (simulated), auto-directing based on role.
- **Citizen Dashboard** (`CitizenDashboardPage.tsx`): Household overview, member badges, Family ID display, document vault status, scheme entitlement breakdown.
- **Household Split Wizard** (`FamilySplitWizardModal.tsx`): Step-by-step modal for selecting members, setting new head, declaring address/income, and receiving instant split approval or officer routing status.
- **Aapnu Mitra AI Chatbot** (`AIChatbotWidget.tsx`): Dark themed floating widget with gold styling for interactive FAQ and guidance.
- **Officer Dashboard** (`OfficerDashboardPage.tsx`): District and pincode dropdown filters, verification queues, split review tools, Plotly analytics charts.
...
# Seed synthetic Gujarat households & demo data
...
## 📑 Full System Requirements Specification & Design Document
...
16. [Tech stack and repository](#16-tech-stack-and-repository)
